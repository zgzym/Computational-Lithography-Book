...
int main()
{
    ...
    float gamma_r_D = 0.001; // weight of discrete penalty function of amplitude mask
    float gamma_a_D = 0.001; // weight of discrete penalty function of phase mask
    float gamma_r_WA = 0.001; // weight of wavelet penalty function of amplitude mask
    float gamma_a_WA = 0.001; // weight of wavelet penalty function of phase mask
    Mat scale; // localized weight matrix of wavelet penalty function
    ...
    // load scale
    string filepath_scale = "../Matrices/txt_version/scale2.txt";
    scale = loadMat(N,N,filepath_scale);
    ...
    Mat dr_WA = Mat::zeros(Size(N,N), CV_32F);

    // Gradient matrix of wavelet penalty of \theta(phase)
    Mat da_WA = Mat::zeros(Size(N,N), CV_32F);
    ...
    while ((sum6 > min_error) && (count < maxloop)){
        ...
        // gradient of wavelet penalty corresponding to amplitude (phi)
        float mr_differ, mi_differ; // 3m1 - m2 - m3 -m4
        float real_exp; // real part of exp(-i*ra)*(3m1-m2-m3-m4)
        for(i=0;i!=N/2;i++){
            for(j=0;j!=N/2;j++){
                // (2*i,2*j)
                mr_differ = 3.0 * mr.at<float>(2*i,2*j) - mr.at<float>(2*i,2*j+1)
                            - mr.at<float>(2*i+1,2*j) - mr.at<float>(2*i+1,2*j+1);
                mi_differ = 3.0 * mi.at<float>(2*i,2*j) - mi.at<float>(2*i,2*j+1)
                            - mi.at<float>(2*i+1,2*j) - mi.at<float>(2*i+1,2*j+1);
                
                real_exp = mr_differ * cos(ra.at<float>(2*i,2*j)) +
                           mi_differ * sin(ra.at<float>(2*i,2*j));

                dr_WA.at<float>(2*i,2*j) = scale.at<float>(2*i,2*j)*(-1) * 
                                           sin(rr.at<float>(2*i,2*j)) * real_exp;
                //(2*i,2*j+1)
                mr_differ = 3.0 * mr.at<float>(2*i,2*j+1) - mr.at<float>(2*i,2*j)
                            - mr.at<float>(2*i+1,2*j) - mr.at<float>(2*i+1,2*j+1);
                mi_differ = 3.0 * mi.at<float>(2*i,2*j+1) - mi.at<float>(2*i,2*j)
                            - mi.at<float>(2*i+1,2*j) - mi.at<float>(2*i+1,2*j+1);
                
                real_exp = mr_differ * cos(ra.at<float>(2*i,2*j+1)) +
                           mi_differ * sin(ra.at<float>(2*i,2*j+1));

                dr_WA.at<float>(2*i,2*j+1) = scale.at<float>(2*i,2*j+1)*(-1) * 
                                           sin(rr.at<float>(2*i,2*j+1)) * real_exp;
                //(2*i+1,2*j)
                mr_differ = 3.0 * mr.at<float>(2*i+1,2*j) - mr.at<float>(2*i,2*j)
                            - mr.at<float>(2*i,2*j+1) - mr.at<float>(2*i+1,2*j+1);
                mi_differ = 3.0 * mi.at<float>(2*i+1,2*j) - mi.at<float>(2*i,2*j)
                            - mi.at<float>(2*i,2*j+1) - mi.at<float>(2*i+1,2*j+1);
                
                real_exp = mr_differ * cos(ra.at<float>(2*i+1,2*j)) +
                           mi_differ * sin(ra.at<float>(2*i+1,2*j));

                dr_WA.at<float>(2*i+1,2*j) = scale.at<float>(2*i+1,2*j)*(-1) * 
                                           sin(rr.at<float>(2*i+1,2*j)) * real_exp;
                //(2*i+1,2*j+1)
                mr_differ = 3.0 * mr.at<float>(2*i+1,2*j+1) - mr.at<float>(2*i,2*j+1)
                            - mr.at<float>(2*i+1,2*j) - mr.at<float>(2*i,2*j);
                mi_differ = 3.0 * mi.at<float>(2*i+1,2*j+1) - mi.at<float>(2*i,2*j+1)
                            - mi.at<float>(2*i+1,2*j) - mi.at<float>(2*i,2*j);
                
    
                real_exp = mr_differ * cos(ra.at<float>(2*i+1,2*j+1)) +
                           mi_differ * sin(ra.at<float>(2*i+1,2*j+1));

                dr_WA.at<float>(2*i+1,2*j+1) = scale.at<float>(2*i+1,2*j+1)*(-1) * 
                                           sin(rr.at<float>(2*i+1,2*j+1)) * real_exp;
            }
        }

        // gradient of wavelet penalty corresponding to phase
        for(i=0;i!=N/2;i++){
            for(j=0;j!=N/2;j++){
                // (2*i,2*j)
                mr_differ = 3.0 * mr.at<float>(2*i,2*j) - mr.at<float>(2*i,2*j+1)
                ...
                real_exp = mi_differ * cos(ra.at<float>(2*i,2*j)) -
                           mr_differ * sin(ra.at<float>(2*i,2*j));

                da_WA.at<float>(2*i,2*j) = scale.at<float>(2*i,2*j)*(-1) * 
                                           sin(rr.at<float>(2*i,2*j)) * real_exp;
                //(2*i,2*j+1)
                mr_differ = 3.0 * mr.at<float>(2*i,2*j+1) - mr.at<float>(2*i,2*j)
                            - mr.at<float>(2*i+1,2*j) - mr.at<float>(2*i+1,2*j+1);
                ...
                da_WA.at<float>(2*i,2*j+1) = scale.at<float>(2*i,2*j+1)*(-1) * 
                                           sin(rr.at<float>(2*i,2*j+1)) * real_exp;
                //(2*i+1,2*j)
                mr_differ = 3.0 * mr.at<float>(2*i+1,2*j) - mr.at<float>(2*i,2*j)
                            - mr.at<float>(2*i,2*j+1) - mr.at<float>(2*i+1,2*j+1);
                ...
                da_WA.at<float>(2*i+1,2*j) = scale.at<float>(2*i+1,2*j)*(-1) * 
                                           sin(rr.at<float>(2*i+1,2*j)) * real_exp;
                //(2*i+1,2*j+1)
                mr_differ = 3.0 * mr.at<float>(2*i+1,2*j+1) - mr.at<float>(2*i,2*j+1)
                            - mr.at<float>(2*i+1,2*j) - mr.at<float>(2*i,2*j);
                ...
                da_WA.at<float>(2*i+1,2*j+1) = scale.at<float>(2*i+1,2*j+1)*(-1) * 
                                           sin(rr.at<float>(2*i+1,2*j+1)) * real_exp;
            }
        }
        ...
        da_WA.convertTo(da_item4,CV_32FC1,gamma_a_WA);
